  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {    
      ...
      if (!taskData || !taskData.records || taskData.records.length === 0) {    
        return api.sendMessage("Timed out after 10 minutes or no records found. Try again later.", threadID, messageID);    
      }    
      ...
      for (let i = 0; i < taskData.records.length; i++) {    
        ...
        await api.sendMessage({ attachment: fs.createReadStream(audioPath) }, threadID);    
      ...
      if (waiting) await api.unsendMessage(waiting.messageID);    
