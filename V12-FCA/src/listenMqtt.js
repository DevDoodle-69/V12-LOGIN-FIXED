...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    if (ctx.mqttClient && ctx.mqttClient.stream) {
        ctx.mqttClient.stream.on('error', function(err) { if (err && err.message && !err.message.includes('readable-stream')) log.error("MQTT stream", err); });
    }
    mqttClient.on('error', function (err) {
        if (err && err.code && ['ECONNREFUSED', 'ETIMEDOUT', 'ENOTFOUND'].includes(err.code)) {
            if (!ctx.mqttErrorLogged) {
                log.error("MQTT connection error", err.code + " - Auto-reconnecting...");
                ctx.mqttErrorLogged = true;
                setTimeout(() => { ctx.mqttErrorLogged = false; }, 60000);
            }
        }
