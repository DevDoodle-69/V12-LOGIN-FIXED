...
module.exports = function (defaultFuncs, api, ctx) {
  return function getThreadList(limit, timestamp, tags, callback) {
    ...
    defaultFuncs
      ...
      .then((resData) => {
        if (resData && resData[resData.length - 1] && resData[resData.length - 1].error_results > 0) throw resData[0].o0.errors;

        if (resData && resData[resData.length - 1] && resData[resData.length - 1].successful_results === 0) throw { error: "getThreadList: there was no successful_results", res: resData };
